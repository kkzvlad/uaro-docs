...
📖 **[Read the Full Halloween Event Guide](#)** *(coming soon)*
...
- **Hornet Pet Egg**
  Fixed issues with Hornet Egg capturing and taming mechanics.

- **Mistress Pet**
  Mistress duplicate pet addition and fixes implemented.

- **Skelion Capture Rate**
  Skelion capture rate set to **1** (extremely rare pet).
...
- **Item Removals:**
  ...
  - Soldier Andre Card removed from OCA pool (due to shield reduction conflict)
  ```
...
- **Halloween Dungeon Balance**
  Reduced mob stats and spawn density in Niflheim Dungeon for improved progression curve.

- **Natural Spawn Functions**
  Moved natural spawned functions to main source code for better performance and maintainability.
...
- **MAX_SKILL_DB Increased**
  Expanded MAX_SKILL_DB constant to accommodate new custom skills.

- **Item Skill Consumption**
  Fixed item skill consumption when using ground-targeted skills to prevent item loss on misclick.

- **Skill Alchemy Output**
  Fixed skill alchemy output message compilation issues.
